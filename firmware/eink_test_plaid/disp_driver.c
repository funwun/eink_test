...
void eink_startup (void) {
    power_on();
}

void eink_shutdown (void) {
    power_off();
}
...
// error with this function where the rows are all loaded into it instantly.
void eink_test_plaid(bool white_background) {
    volatile uint16_t xpos;
    volatile uint16_t ypos;
    volatile uint8_t onColor;
    volatile uint8_t offColor;
    
    if ( TRUE == white_background ) {
        onColor = BYTE_BLACK;
        offColor = BYTE_WHITE;
    } else {
        onColor = BYTE_WHITE;
    ...
    while ( ypos < 600 ) {
        ...
        if ( ( ypos % 32 ) < 8 ) {
            ...
            while ( xpos < 200 ) {
                // fill this row with onColor
                setpins_data(onColor);
